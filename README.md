...
| ⏱️ **任务队列**        | 同一时间只允许一个任务运行，避免并发冲突；SQLite 持久化存储，服务重启自动恢复队列状态，保留完整历史记录                            |
...
| 🔒 **单会话机制**      | 同一用户仅允许一个活跃会话，新登录自动踢掉旧会话，被踢出用户收到明确提示                    |
...
| 日志历史 | 任务记录表格（分页展示，每页 10 条）+ 脚本日志文件查看 + 日志归档功能（按时间戳归档保存） |
| 审计日志 | 记录管理员所有关键操作，记录操作员 IP 地址，支持按操作人、事件类型、时间等筛选（仅限 `system_admin`） |
| 用户管理 | 个人资料管理（头像、密码）、管理员可管理用户角色与账户状态        |
| 系统设置 | 在线编辑 config.toml、SSH 配置管理（仅 `system_admin` 可访问，二次确认防误操作）            |
