...
编辑 `script/config.ini`，确认 SSH 段密钥路径指向服务器上的实际路径：
...
```bash
docker ps -f name=deploy-tool        # 查看状态
docker logs -f deploy-tool           # 实时日志
docker start deploy-tool             # 启动
docker stop deploy-tool              # 停止
docker stop deploy-tool && docker rm deploy-tool  # 删除重建（更新镜像或修改环境变量后用）
```

> `script/config.ini` 直接挂载进容器，**修改后立即生效**，无需重启容器。如需修改环境变量（如密码），则需删除并重建容器。
...
| `LOG_BASE_DIR` | 未设置（退回 `SCRIPT_DIR`） | 日志文件存放目录（Docker 下设为 `/app/logs`，对应 volume 挂载路径） |
...
宿主机的 `~/.ssh` 目录以只读方式挂载到容器内 `/root/.ssh`，容器以 root 用户运行（`-u root`）。`SKIP_SSH_INIT=1` 环境变量已在启动命令中预设，脚本会直接使用 `-i /root/.ssh/id_rsa` 参数进行 SSH 连接，无需 ssh-agent。
