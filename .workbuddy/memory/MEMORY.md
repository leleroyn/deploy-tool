...
- config.toml 和 script/ 目录通过 volume 挂载，不打包进镜像；config.toml 不能只读挂载（Web 设置页需写入）
- **生产部署用 docker run（不用 docker compose）**，以 root 用户运行（-u root）
- SSH 密钥挂载：`/root/.ssh:/root/.ssh:ro`，config.toml 里 `key = /root/.ssh/id_rsa`
- 日志目录：宿主机提前 `mkdir -p logs`，挂载 `./logs:/app/logs`，ENV LOG_BASE_DIR=/app/logs
- 配置文件：根目录 `config.toml`（不是 config.ini），挂载到容器内 `/app/config.toml`
...
- 完整启动命令：
  ...
  docker run -d --name deploy-tool --restart unless-stopped -p 3001:3001 -u root \
    -e DEPLOY_PASSWORD=密码 -e SCRIPT_DIR=/app/script -e CONFIG_FILE=/app/config.toml \
